<!-- select2 from builder -->
<?php
// TODO, es una proposta el "value" del <select><option>
//'format' => '%name% (%address%, %city%)',

$field['options'] = $field['builder']->get()->keyBy($field['key'])
    ->map(function ($item) use ($field) {

        $option[$field['attribute']] = $item->{$field['attribute']};

        if(isset($field['data-attributes'])){
            foreach ($field['data-attributes'] as $key => $value) {
            ...
            @if (count($field['options']))
                @foreach ($field['options'] as $key => $value)
                    <option value="{{ $key }}"
                        ...
                        @if(isset($field['data-attributes']))
                            @foreach($field['data-attributes'] as $dataKey => $dataValue)
                                data-{{ $dataKey }}="{{ $value[$dataKey] }}"
                            @endforeach
                        @endif

                    >{{ $value[$field['attribute']] }}</option>
                @endforeach
            @endif
    </select>
