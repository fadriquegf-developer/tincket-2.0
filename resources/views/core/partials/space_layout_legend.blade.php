<div class='space-layout-legend'>
    <ul class='legend-labels'>
        ...
        @if(isset($zones_map) && count($zones_map) > 1)
            @foreach($zones_map as $zone)
                <li>
                    <span style="display: inline-block; width: 16px; height: 16px; border-radius: 50%; background: white; border: 2px solid {{ $zone['color'] }}; margin-right: 6px;"></span>
                    {{ $zone['name'] }}
                </li>
            @endforeach
        @endif
