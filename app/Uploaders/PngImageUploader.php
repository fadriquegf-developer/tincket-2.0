 ...
 * PngImageUploader
 * --------------------------------------------------------------------------
 * Similar a WebpImageUploader pero genera PNG en lugar de WebP.
 * Útil para imágenes que se usarán en PDFs (wkhtmltopdf no soporta WebP).
 *
 ...
 */
class PngImageUploader extends SingleFile
{
    ...
    public function uploadFiles(Model $entry, $value = null)
    {
        ...
        if (
            ...
            // 1. Guardar imagen principal en PNG
            Storage::disk($this->getDisk())
                ->put($path . $fileName, $img->encode(new PngEncoder()));

            // 2. Conversiones (también en PNG)
            foreach ($this->conversions as $suffix => $w) {
                $variant = (clone $img)->scale(width: $w);
                Storage::disk($this->getDisk())
                    ->put($path . "{$suffix}-{$fileName}", $variant->encode(new PngEncoder()));
            }
